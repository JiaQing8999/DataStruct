...
public class BinarySearchTree<T extends Comparable<T>> implements BinarySearchTreeInterface<T> {
    ...
    /**
     * Task: Adds newEntry to the nonempty sub tree rooted at rootNode.
     */
    private T addEntry(Node rootNode, T newEntry) {
        T result = null;
        int comparison = newEntry.compareTo(rootNode.data);

        // newEntry matches entry in root
        if (comparison == 0) {			
    ...
    /**
     * Task: Removes the entry in a given root node of a sub tree.
     *
     * @param rootNode the root node of the sub tree
